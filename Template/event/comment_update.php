<div class="activity-comment-wrapper">
    ...
    <span class="comment-original-dates">
        <span class="deleted-wrapper" title="<?= t('Showing Updated Content') ?>"><?= t('Updated Comment') ?></span>
        ...
        <?php if (!empty($this->dt->datetime($comment['date_modification']))): ?>
            <span class="original-modified" title="<?= $this->dt->datetime($comment['date_modification']) ?>LT">
    ...
    <p class="activity-title activity-comment-title">
        <?= e('%s updated a comment on task %s', $this->text->e($author), $this->url->link(t('#%d', $task['id']), 'TaskViewController', 'show', array('task_id' => $task['id'], 'project_id' => $task['project_id']), false, 'task-comment-link', t('View Task'))) ?>
